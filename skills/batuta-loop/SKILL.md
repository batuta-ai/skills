...
1. **Preflight.** `command -v batuta` — absent → say the core binary is missing, give the install line from the README, offer the interactive cycle instead. Require core `v1.1.0-beta.23` or newer and `batuta capabilities 2>/dev/null | grep -q '"loop"'`; an older core must be updated through the project's chosen install path, but never install or update it without authorization. Then `batuta doctor`: hosts, adapters, clean tree, test command, skills version.
...
7. **Recover review.** For a pending gate, use `batuta loop --resume <delivery>` or `batuta loop --roadmap`; inspect it without execution using `batuta loop --supervise <delivery> --review-status`. Never auto-accept, reset the attempt budget, or replay a failed or uncertain reviewer. After reconciling ownership and retained evidence, only an authorized conductor may run `--review-judgment accept|reject` with the exact `--review-id`, `--review-digest`, and nonempty `--rationale`. `SHIP` clears progression only; it never authorizes merge or publication.
