...
4. **Launch.** `batuta loop .batuta/plan-<slug>.md` through the runtime's background facility, or tell the user to run it in another terminal. The run ends the turn: report the journal path (`.batuta/journal/<delivery>.jsonl`) and how to watch: `batuta loop --dashboard --watch` redraws a live panel (task, lane, executor, state, elapsed, criteria done from the executor's `BATUTA-PROGRESS` lines) until the run ends; `--dashboard` alone prints one TSV snapshot.
