...
2. **Pick the plan.** The user names it or there is exactly one `.batuta/plans/*.md` (or legacy `.batuta/plan-*.md`) with `Status: approved`. A plan not approved → stop; approval happens in `/batuta-plan`.
