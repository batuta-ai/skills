...
3. **Walk the journey.** Follow the enter-act-verify-capture loop in [references/session.md](references/session.md), including the charter's `Must try`, declared tour, time box, branches, and abandonment through the true end state; deliver a verdict and evidence at each checkpoint or divergence.
...
5. **Write the debrief.** Follow [the writeback contract](references/session.md) after each session and update the dated report with observations, evidence paths, paper cuts, bug ids, and its ledger verdict; deliver resumable on-disk state with no completed row still planned.
6. **Close the round.** Reconcile scenario verdicts with [the planner's status enums](../batuta-qa-plan/references/tree.md) and bugs with [the shared registry](../batuta-qa-plan/references/bugs.md), then run the project's required gate; deliver a final readiness decision with totals by shared impact tier and every skip or blocked prerequisite disclosed.
