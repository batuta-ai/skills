...
```
   ...
   Machine contract (read by `/batuta-loop`): a task is a `- [ ] N. <title> — <domain>/<complexity>` line, the `→ <executor/model>` tail optional and advisory — the routing table decides; the lane is mandatory because the loop routes by it. `Accept:` is mandatory: entries split on `;`, each `<criterion> → <proof>` where the proof is a command the loop runs in the task's worktree (exit 0 = holds, so no `;` inside it); a criterion without an arrow is judged by the read-only verifier. `Depends on:` lists task numbers, in any order; `Scope:` is a comma-separated list of paths or globs contained in the repository (no absolute paths, no `..`). `**Status:**` is exactly one of the four words, on the `**Created:**` line, once. Everything else is prose. `## Decisions and context` reaches the executors' briefs: a paragraph that starts with `**Task N.**` or `**Tasks N–M.**` (hyphen, comma lists such as `**Tasks 1, 3–4.**`) goes only to those tasks' briefs; unlabelled paragraphs go to every brief. A label naming a task the plan does not define fails the parse. Write a decision for one task under its label, so a small model never applies another task's decision as its own.
5. **Self-check:**
   ```bash
